...
DEFAULT_PREBUILT_ASSET = "dfttest-msys2-ucrt64.zip"
...
def _default_prebuilt_url(version: str) -> str:
    ...
    asset = os.environ.get("DFTTEST_PREBUILT_ASSET_NAME") or DEFAULT_PREBUILT_ASSET
...
def _stage_prebuilt_plugin(version: str, target_dir: Path) -> bool:
    ...
    if not _supports_prebuilt():
        print("DFTTest wheel build: prebuilt release asset path only applies to Windows x86_64; falling back to local build")
        return False
