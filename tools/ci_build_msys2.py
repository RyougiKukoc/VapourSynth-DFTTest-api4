...
def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(description="Build and package DFTTest with MSYS2 UCRT64.")
    ...
    if "CC" not in env:
        env["CC"] = find_tool("gcc")
    if "CXX" not in env:
        env["CXX"] = find_tool("g++")
    if "PKG_CONFIG" not in env:
        env["PKG_CONFIG"] = find_tool("pkg-config")
    ...
    dll = build_dir / "libdfttest.dll"
    if not dll.exists():
        dll = build_dir / "dfttest.dll"
    if not dll.exists():
        raise FileNotFoundError(build_dir / "libdfttest.dll")

    shutil.copy2(dll, pkg_dir / "dfttest.dll")
